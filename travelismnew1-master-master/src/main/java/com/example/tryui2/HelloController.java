...
public class HelloController {
    ...
    @FXML
    private Label flightnolbl,departurelbl,arrivallbl,classlbl ;
...
try{
   usernow.setText(tmpU);
        ...
        void confirmbookingf (MouseEvent event) throws Exception {
            String c=arrivallbl.getText();
            for(int i=0;i<Program.getFlights().size();i++){
                if(c.equals(Program.getFlights().get(i).getArriving_Airport()));
                    Program.getTourists().get(0).additem(Program.getFlights().get(i));
            }
...
//            outputflight.setTextFill(Paint.valueOf("Black"));
            ...
            /**/
            flightnolbl.setText(flightnumber);
            departurelbl.setText((depair));
            arrivallbl.setText(Arrair);
            classlbl.setText(aclass);
